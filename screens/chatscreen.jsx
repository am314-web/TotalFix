...
const CHAT_SEEN_KEY_PREFIX = "chat_seen_client_";
...
export default function ChatScreenApp() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const professionalIds = Array.from(
        new Set(list.map((b) => b.professional_id).filter(Boolean))
      );
      let professionalNameMap = {};
      if (professionalIds.length > 0) {
        ...
        for (const p of profileRows || []) {
          professionalNameMap[p.id] = p.name || "Professional";
        }
      }
      ...
      if (ids.length > 0) {
        ...
        for (const row of lastRows || []) {
          if (!lastMap[row.booking_id]) lastMap[row.booking_id] = row;
        }
      }

      const normalized = list.map((b) => ({
        bookingId: b.id,
        peerId: b.professional_id,
        title: professionalNameMap[b.professional_id] || "Professional",
        subtitle: `${b.scheduled_date || "Date"} | ${b.scheduled_time || "Time"}`,
        lastMessage: lastMap[b.id]?.message || "Start conversation",
      }));
      ...
      if (mounted) {
        const map = {};
        for (const row of normalized) {
          map[row.peerId] = await getUserAvatar(row.peerId, "professional");
        }
        setAvatarMap(map);
        setInbox(normalized);
        setUnreadMap(nextUnreadMap);
        if (routeBookingId) {
          const found = normalized.find((x) => x.bookingId === routeBookingId);
    ...
    load();
    channel = supabase
      .channel("chat-client-inbox-updates")
  ...
  useEffect(() => {
    ...
    channel = supabase
      .channel(`chat-client-${active.bookingId}`)
      .on(
        "postgres_changes",
      ...
      <ScrollView contentContainerStyle={s.scroll}>
        {inbox.map((item) => (
          <TouchableOpacity key={String(item.bookingId)} style={s.row} onPress={() => setActive(item)} activeOpacity={0.8}>
            ...
            <Text style={s.rowMsg} numberOfLines={1}>{item.lastMessage}</Text>
            </View>
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {active ? renderConversation() : renderInbox()}
      <AppBottomTab />
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  rowMsg: { fontSize: sc(12), color: C.labelGray, marginTop: 6 },
